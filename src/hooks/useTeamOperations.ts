...
import { createNewTeam, addMembersToTeam, removeMemberFromTeam } from '../utils/teamUtils';
...
export const useTeamOperations = () => {
  ...
  const handleCreateTeam = useCallback((values: Partial<Team>) => {
    try {
  ...
  const handleUpdateTeam = useCallback((values: Team) => {
    try {
  ...
  const handleAddMembers = useCallback((userIds: string[]) => {
    if (selectedTeam) {
      try {
